...
class Flow(object):
    ...
    def extract(self, bursts):
        """Extract flows from bursts.
            ...
            result : dict
                ...
                Flow tuple is defined as (timestamp, src, sport, dst, dport, prot)
    ...
    def extract_single(self, burst):
        ...
        for packet in burst:
            # Define key as 6-tuple (burst, src, sport, dst, dport, prot)
            key, incoming = self.key(timestamp, packet)

            # Set length depending on incoming or outgoing
            length = -packet[6] if incoming else packet[6]
    ...
    def key(self, timestamp, packet):
        """Extract the key of a packet and check whether it is incoming or
           outgoing.
            ...
            packet : np.array of shape=(n_features)
                Packet representation [timestamp, src, dst, sport, dport, prot, len]
        ...
        key = (timestamp, ipaddress.ip_address(int(packet[1])), packet[3],
                          ipaddress.ip_address(int(packet[2])), packet[4],str(packet[5]))
        ...
        if incoming:
            key = (key[0], key[3], key[4], key[1], key[2], key[5])

        # Set IP addresses to string
        key = (key[0], str(key[1]), key[2], str(key[3]), key[4],key[5])
